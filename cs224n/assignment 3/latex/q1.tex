...
\end{lstlisting}
    ...
    ANSWERS: The masks are used to exclude the padded elements from attention computation. This is done by setting the values of $e_t$ corresponding to those elements to -inf. This is done because the padded elements have no meaning and should not be attended to by the encoder.
\end{parts}
...
\end{lstlisting}
    ...
    ANSWER: Dot Product Attention - more computational efficient but does not allow learning from multiple weights (limit in capturing patterns). On the opposite side, Additive Attention - learn weights for different set of weights but is more expensive to use.
