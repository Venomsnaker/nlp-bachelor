...
\begin{subparts}
    ...
    \ifans{
        \newline The conditions for the categorical distribution $ \alpha $
         to puts almost all of its weight on some $ \alpha_j $ :
        $ k_j^T q \gg k_i^Tq, \; \forall i \neq j $ 
    }

    \subpart[1] Under the conditions you gave in (i), \textbf{describe} the output $c$. 

    \ifans{
        \newline $ \alpha_i > 0, \; \sum{\alpha_i} = 1, \; \alpha_j \approx 1 
        \Rightarrow \sum{v_i \alpha_i} \approx v_j \alpha_j \approx v_j $ 
    }
...
\ifans{
    \newline Since we want to incorporate information from two vectors: $ k_a^T q = k_b^T q \gg k_i^T q \; \forall i \neq a, b $
    \newline With $ c = \sum v_i \alpha_i = v_a \alpha_a + v_b \alpha_b \approx \dfrac{1}{2}(v_a + v_b)$
    \newline Consider $ k_a^T q = k_b^T q = \beta, \; \alpha_a = \alpha_b \approx \dfrac{exp(\beta)}{2exp(\beta)} $
    \newline For $ \beta \gg 0 \Rightarrow exp(\beta) \to \infty \Rightarrow \alpha_a = \alpha_b \approx \dfrac{exp(\beta)}{2 exp(\beta)} = \dfrac{1}{2} $
    \newline So: $ q = 2\beta (k_a + k_b) \approx \beta (k_a + k_b)$ with $ \beta \gg 0 $
    }
...
\ifans{
    \newline Since $ \alpha $ is vanishingly small, covariance matrices (diagonal) are vanishingly small also, and $ k_i \approx u_i $
    \newline Besides that, we assume that the means $ \mu_i $ are all perpendicular; so the results are similar to solution (b).
}
...
\ifans{
    \newline We have $ \mu_i^T \mu_i = 1 $ and $ \alpha $ is vanishingly small
    $ \Rightarrow \sum_a = \alpha I + \dfrac{1}{2} (\mu_a \mu_a^T) \approx \dfrac{1}{2} \Rightarrow k_a \in [0.5 \mu_a, 1.5 \mu_a ]$
    \newline So: $ k_a = \gamma \mu_a $ where $ \mu_a \backsim N(1, 0.5) $ 
    \newline We have: $ k_a^T q \approx \gamma \beta , \; \beta \gg 0 $, $ k_b^T q \approx \beta , \; \beta \gg 0 $, and $ k_i^T q \approx 0 $ (Question 1.b)
    \newline $ \alpha_a = \dfrac{exp(\gamma \beta)}{exp(\gamma \beta) + exp(\beta)} = \dfrac{1}{1+exp(\beta(1 - \gamma))} $
    \newline $ \alpha_b = \dfrac{exp(\beta)}{exp(\gamma \beta) + exp(\beta)} = \dfrac{1}{1 + exp(\beta (\gamma - 1))} $
    \newline With $ \gamma \to 0.5 $ and $ \beta \gg 0 $, : $\alpha_a \approx 0 $ while $\alpha_b \approx 1$
    \newline With $ \gamma \to 1.5 $ and $ \beta \gg 0 $, : $\alpha_a \approx 1 $ while $\alpha_b \approx 0$
    \newline Therefore: $c = v_b$ when $\gamma \to 0.5$ and $c = v_a$ when $\gamma \to 1.5 \; (\beta \gg 0)$
}   
\end{subparts}
...
\ifans{
    \newline If $c_1 = \dfrac{1}{2} (v_a + v_b) $ for $ q_1 = \beta (\mu_a + \mu_b)$ and $c_2 = \dfrac{1}{2} (v_a + v_b) $ for $ q_2 = \beta (\mu_a + \mu_b) $
    \newline With $c = \dfrac{1}{2}(c_1 + c_2) = \dfrac{1}{2} (v_a + v_b) $
}
...
\ifans{
    \newline We chose $q = \beta (\mu_a + \mu_b):$
    \newline $ c \approx \dfrac{1}{1 + exp(\beta(1 - \gamma))} v_a + \dfrac{1}{1 + exp(\beta(\gamma - 1))} v_b $ (similar conditions to c.ii)
    \newline When including more attention heads, with $ \gamma \to 1 , \; c = \dfrac{1}{2} (v_a + v_b) $
}
\end{subparts}
...
\ifans{
    \newline By adding more attention heads, c won't oscillate as much between $v_a$ and $v_b$ as before since $\gamma$ is moving toward its mean 1.
}
