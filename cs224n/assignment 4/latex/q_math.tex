...
\ifans{
    ...
    \newline With $ k_a^T q = k_b^T q = \beta, \; \alpha_i \approx \dfrac{exp(\beta)}{2exp(\beta)} $
    \newline For $ \beta \gg 0 \Rightarrow exp(\beta) \to \infty \Rightarrow \alpha_i \approx \dfrac{exp(\beta)}{2 exp(\beta)} = \dfrac{1}{2} $
    \newline So: $ q = 2\beta (k_a + k_b) $ with $ \beta \gg 0 $
    }
...
\ifans{
    ...
    \newline Besides that, we assume that the means $ \mu_i $ are all perpendicular; so the results is similar to solution 1.b.
}
