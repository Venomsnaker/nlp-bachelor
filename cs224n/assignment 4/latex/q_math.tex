...
\ifans{
    \newline $ c \approx \dfrac{1}{2}(v_a + v_b) \Rightarrow \alpha_a = \alpha_b = \dfrac{1}{2} $
    \newline This means: $ k_a^T q = k_b^T q \gg k_i^T q \; \forall i \neq a, b $
    \newline With $ k_a^T q = k_b^T q = \beta, \; \alpha_a = \alpha_b \approx \dfrac{exp(\beta)}{2exp(\beta)} $
...
\ifans{
    \newline Since $ \alpha $ is vanishingly small, covariance matrices (diagonal) is vanishingly small also, and $ k_i \approx u_i $
    \newline Besides that, we assume that the means $ \mu_i $ are all perpendicular; so the results is similar to solution (b).
}
...
\ifans{
    ...
    \newline So: $ k_a = \gamma \mu_a $ where $ \mu \backsim N(1, 0.5) $ and $ k_i = \mu_i, \; \forall i \neq a $
    \newline We have: $ k_a^T q \approx \gamma \beta , \; \beta \gg 0 $, $ k_b^T q \approx \beta , \; \beta \gg 0 $, and $ k_i^T q \approx 0 $
