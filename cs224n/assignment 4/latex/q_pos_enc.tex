...
\begin{parts}
    ...
    \begin{subparts}
        ...
        \ifans{
            \newline $ H_{perm} = softmax(\dfrac{PQ (PK)^T}{\sqrt{d} }) PV = PH $
            \newline $ Z_{perm} = ReLU(P H W_1 + l b_1)W_2 + l b_2  = PZ $
        }
        ...
        \ifans{
            \newline Using a permutation matrix P with input X when building position embeddings in transformers leads to the model position context based purely on P, not capturing all the possible meanings.
        }
    \end{subparts}
    ...
    \begin{subparts}
        ...
        \ifans{
            \newline No, since it still doesn't capture all the possible positional meanings of the input. Though the fixed function can be tuned for better results.
        }
        ...
        \ifans{
            \newline No, since the position embeddings are generated using sine and cosine functions that depend on the position t of the token within the sequence. 
            \newline These trigonometric functions ensure that each position t generates a unique embedding across the different dimensions i.
        }
    \end{subparts}
\end{parts}
