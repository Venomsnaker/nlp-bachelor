...
\end{lstlisting}
    ...
    \textcolor{Red}{The masks are used to exclude the padded elements from attention computation.
    This is done by setting the values of $e_t$ corresponding to those elements to $- \infty$. 
    \newline It is necessary to use the masks in this way because the padded elements contribute no meanings and should not be attended to by the encoder.}
\end{parts}
...
\end{lstlisting}
    ...
    \end{subparts}
    \textcolor{Red} {
        Dot Product Attention: more computationally efficient but does not allow learning from multiple weights (limit in capturing patterns).
        \newline Additive Attention: learn weights for different sets of weights but it is more expensive to compute.
    }
\end{parts}
