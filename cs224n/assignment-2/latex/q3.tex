...
\begin{parts}
    ...
    [ROOT] & [I, presented, my, findings, at, the, NLP, con] &  & Initial Configuration \\
    $[$ROOT, I] & [presented, my, findings, at, the, NLP, con] &  &  \texttt{SHIFT}  \\
    $[$ROOT, I, presented] & [my, findings, at, the, NLP, con] &  &  \texttt{SHIFT}  \\
    $[$ROOT, presented] & [my, findings, at, the, NLP, con] & presented$\to$I &  \texttt{LEFT-ARC}  \\
    $[$ROOT, presented, my] & [findings, at, the, NLP, con] & & \texttt{SHIFT} \\
    $[$ROOT, presented, my, findings] & [at, the, NLP, con] & & \texttt{SHIFT} \\
    $[$ROOT, presented, findings] & [at, the, NLP, con] & findings$\to$my & \texttt{LEFT-ARC} \\
    $[$ROOT, presented] & [at, the, NLP, con] & presented$\to$findings & \texttt{RIGHT-ARC} \\
    $[$ROOT, presented, at] & [the, NLP, con] & & \texttt{SHIFT} \\
    $[$ROOT, presented, at, the] & [NLP, con] &  & \texttt{SHIFT} \\
    $[$ROOT, presented, at, the, NLP] & [con] &  & \texttt{SHIFT} \\
    $[$ROOT, presented, at, the, NLP, con] & & & \texttt{SHIFT} \\
    $[$ROOT, presented, at, the, con] & & con$\to$NLP & \texttt{LEFT-ARC} \\
    $[$ROOT, presented, at, con] & & con$\to$the & \texttt{LEFT-ARC} \\
    $[$ROOT, presented, con] & & con$\to$at & \texttt{LEFT-ARC} \\
    $[$ROOT, presented] & & presented$\to$con & \texttt{LEFT-RIGHT} \\
    $[$ROOT] & & ROOT$\to$presented & \texttt{LEFT-RIGHT}
    \end{tabular}\newline
    \end{adjustwidth}
    ...
    \ifans{
        \newline SHIFT into stack: n and LEFTARC or RIGHTARC: n
        \newline The number of steps: 2n
    }
    ...
    \begin{enumerate}[label=\roman*.]
        ...
        \newline \ifans{
            $ \dfrac{\partial h_i}{\partial x_j}  = \dfrac{\partial ReLU(x_i W + b_1)}{\partial x_j} = \dfrac{\partial ReLU(x_i W + b_1)}{\partial x_i}\dfrac{\partial x_i}{\partial x_j} = W $
        }
        ...
        \newline \ifans{
            \newline $ \dfrac{\partial CE(y, \hat{y})}{\partial l_i} = \sum_j \dfrac{\partial CE(y, \hat{y})}{\partial \hat{y}_j} \dfrac{\partial \hat{y}_j}{\partial l_i}$
            \newline $ = \sum_j \dfrac{\partial -\sum^3_{j=1} y_j log(\hat{y}_j)}{\partial \hat{y}_j} \dfrac{\partial \hat{y_j}}{\partial l_i}   $
        }
...
\ifans{
    ...
    \begin{itemize}
        ...
        \item \textbf{Incorrect dependency}: aquisition $\rightarrow$ citing
    ...
    \begin{itemize}
        \item \textbf{Error type}: Prepositional Phrase Attachment Error
        \item \textbf{Incorrect dependency}: Canada $\rightarrow$ Investment
        \item \textbf{Correct dependency}: declined $\rightarrow$ Investment
    \end{itemize}
...
\newline \ifans{ \newline It can help the model identify the function of each word and calculate the probability when predicting (especially useful when dealing with words that have multiple meanings). This also provides additional information about the syntactic and semantic contexts.}
\end{parts}
