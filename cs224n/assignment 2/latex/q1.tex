...
\ifans{
    \newline \( \dfrac{\partial J_{naive-softmax}(v_c, o, U)}{\partial u_w}
    = \dfrac{\partial}{\partial u_w} [log(\sum_{w \in Vocab}exp(u_w^T v_c)) - u_o^T v_c] \)
    \newline \( = -y_w v_c + \dfrac{log \sum_{w \in Vocab} exp(u_w^T v_c) . v_c}{\sum_{w \in Vocab} exp(u_w^T v_c) v_c}\)
    \newline \( = -y_w v_c + \hat{y}_w v_c\ = (\hat{y}_w - y_w)^T v_c \)
    \newline where \( y_w = 
        \begin{cases}
            1 & w = o\\
            0 & w \neq o
