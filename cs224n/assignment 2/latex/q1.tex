...
\ifans{
    ...
    \newline Meanwhiles, models with smaller word vector dimension will be affected heavily. Since this usage of L2 normalized vectors will smooth out severely U.  
}
...
\ifans{
    ...
    \newline \( = \dfrac{ exp(u_w^T v_c) v_c}{\sum_{w \in Vocab} exp(u_w^T v_c)} - y_w v_c \)
    \newline \( = \hat{y}_w v_c\ - y_w v_c = (\hat{y}_w - y_w) v_c \)
    \newline where \( y_w = 
        \begin{cases}
            1 & w = o\\
            0 & w \neq o
...
\ifans {
    \newline \( \dfrac{\partial J_{naive-softmax}(v_c, o, U)}{\partial u_w} = (\hat{y}_w - y_w) v_c \)
