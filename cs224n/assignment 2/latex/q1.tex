...
\ifans{
    ...
    \newline \( = \sum_{x \in Vocab} \dfrac{exp(u_x^T v_c).u_x }{\sum_{w \in Vocab} exp(u_w^T v_c)} - u_o\)
    \newline \( = \sum_{x \in Vocab} P(O = x|C = c).u_x - u_o \)
    \newline \( = U \hat{y} - U y\)
    \newline \( = U(\hat{y} - y) \)
    \newline QUESTION II. \( \hat{y} = y \) (The model can make perfect prediction on outside words o from center word c)
    \newline QUESTION III. Subtracting this graident from \(v_c\) help minimize the loss function (negative partial derivative values point toward minima).
}
...
\ifans{
    ...
    \newline Meanwhiles, models with smaller word vector dimension will be affected heavily.
}
...
\ifans{
    \newline \newline \( \dfrac{\partial J_{naive-softmax}(v_c, o, U)}{\partial U}
    = \dfrac{\partial}{\partial U} [log(\sum_{w \in Vocab}exp(u_w^T v_c)) - u_o^T v_c] \)
    \newline \( = \dfrac{1}{\sum_{w \in Vocab} exp(u_w^T v_c)} \sum_{w \in Vocab} exp(u_w^T v_c) \dfrac{\partial u_w^T v_c}{\partial U} - \dfrac{\partial u_o^T v_c}{\partial U}  \)
    \newline \( = \sum_{x \in Vocab} \dfrac{exp(u_x^T v_c)}{\sum_{u \in Vocab} exp(u_w^T v_c)} \dfrac{\partial u_w^T v_c}{\partial U} - \dfrac{\partial u_o^T v_c}{\partial U} \)
    \newline \( = \sum_{w \in Vocab} \hat{y}_w \dfrac{\partial u_w^T v_c}{\partial U} - \dfrac{\partial u_o^T v_c}{\partial U} \)
    \newline \( = v_c \hat{y}^T - v_c y^T\)
    \newline \( = v_c (\hat{y} - y)^T \)
    }
...
\ifans {
    \newline \( \dfrac{\partial J_{naive-softmax}(v_c, o, U)}{\partial U} = \begin{bmatrix} \hat{y}_1 v_c & \hat{y}_2 v_c & ... & \hat{y}_o v_c - v_c & ... & \hat{y}_{|Vocab|} v_c\end{bmatrix} \)
    \newline \(=  v_c \hat{y}^T - v_c y^T\)
    \newline \(= v_c (\hat{y} - y)^T\)
}
