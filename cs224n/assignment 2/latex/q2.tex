    ...
    \part[4] Adam Optimizer\newline
        ...
        \begin{subparts}
            ...
            \subpart[2]First, Adam uses a trick called {\it momentum} by keeping track of $\bm$, a rolling average of the gradients:
                ...
                \ifans{
                    \newline m (controlled by parameter \( \beta_1 \)) will adjust the next $ \theta $ by the previous one to make sure that each jump is not too drastic. This implementation also averages out updates, reducing their variance.
                }   
            ...
            \subpart[2] Adam extends the idea of {\it momentum} with the trick of {\it adaptive learning rates} by keeping track of  $\bv$, a rolling average of the magnitudes of the gradients:
                ...
                \ifans{
                    \newline Parameters with smaller gradient will receive relatively larger updates. This can be beneficial to escape local minimum.
                }
          ...
          \subpart[2] Why should dropout be applied during training? Why should dropout \textbf{NOT} be applied during evaluation? \textbf{Hint:} it may help to look at the dropout paper linked. \newline
            ...
            \ifans{
                \newline QUESTION I. $ \gamma $  should be equal because the model needs to randomly set units in the hidden layer h.
                \newline QUESTION II. Dropout should be applied during training to avoid overfitting. While it's a bad idea to apply it in evaluation since it will destroy ebstablished layers connection.
            }
        \end{subparts}
